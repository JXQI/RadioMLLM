    ...
    for idx, each_data in enumerate(j_data):
        ...
        if data_type == "test":
            condition = each_data["answer_type"] == "CLOSED" and (
                each_data["phrase_type"] == "test_para"
                or each_data["phrase_type"] == "test_freeform"
            )
        else:  # train
            condition = (
                each_data["phrase_type"] != "test_para"
                and each_data["phrase_type"] != "test_freeform"
            )
        ...
        if condition:
            ...
            if (
                "question_rephrase" in each_data
                and each_data["question_rephrase"] != "NULL"
            ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process RadVQA data for instruction tuning."
    )
