...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Root path where all parquet files are located",
)
parser.add_argument(
    "--output_path",
    type=str,
    required=True,
    help="Root path where processed dataset will be saved",
)
...
for split, parquet_files in splits.items():
    ...
    image_bytes = split_data["image"].apply(
        lambda x: x["bytes"]
    )  # Extract 'bytes' from the dictionary
    image_paths = split_data["image"].apply(
        lambda x: x["path"]
    )  # Extract 'path' from the dictionary
    ...
    for i, (img_bytes, img_path) in tqdm(
        enumerate(zip(image_bytes, image_paths)),
    ...
    metadata = pd.DataFrame(
        {"file_name": image_paths, "question": questions, "answer": answers}
    )
